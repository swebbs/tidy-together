...
interface ChoreRecord {
  ...
  needsImprovementAt?: number;
  needsImprovementBy?: Person;
...
function render(): void {
  ...
  $('#app').innerHTML = `<main class="shell">
    ...
    <div class="legend muted"><span>● To do</span><span class="waiting">● Awaiting verify</span><span class="needs-improvement waiting">● Needs improvement</span><span class="verified">● Verified</span></div>
...
function renderWeek(start: Date): string {
  ...
  const entries = (kind: BucketName, person: Person): string => buckets[kind].chores.map((chore) => { const key = `${id}-${kind === 'cooking' ? 'cook' : 'clean'}-${chore}`; const r = state.records[key] || {}; const canDo = state.selected === person && (!r.doneAt || (r.doneAt && !r.verified && r.doneBy === state.selected)); const canVerify = state.selected !== person && !!r.doneAt && !r.verified && !r.needsImprovement; const verificationLabel = r.autoVerified ? 'Auto-verified' : `Verified by ${esc(r.verifiedBy)}`; return `<label class="chore ${r.verified ? 'is-verified' : r.needsImprovement ? 'is-needs-improvement' : r.doneAt ? 'is-waiting' : ''}"><input type="checkbox" data-key="${key}" ${r.doneAt ? 'checked' : ''} ${canDo || canVerify ? '' : 'disabled'}><span>${esc(chore)}</span>${r.doneAt ? `<small>Done by ${esc(r.doneBy)} · ${r.verified ? `${verificationLabel} · ` : r.needsImprovement ? `Needs improvement · ` : 'Waiting for verification · '}${formatTime(r.verifiedAt || r.needsImprovementAt || r.doneAt)}</small>` : ''}${canVerify ? `<button type="button" class="needs-improvement-button" data-action="needs-improvement" data-key="${key}">Needs improvement</button>` : ''}</label>`; }).join('');
...
async function markNeedsImprovement(key: string): Promise<void> { const old = state.records[key]; if (!old?.doneAt || old.verified || !state.selected || old.doneBy === state.selected) return; await save(key, { ...old, verified: false, needsImprovement: true, needsImprovementAt: Date.now(), needsImprovementBy: state.selected }); }
